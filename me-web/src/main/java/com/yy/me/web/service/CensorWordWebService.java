...
public class CensorWordWebService {
    ...
    /**
     * 检查文字内容中是否含有敏感词，不包含则返回true，包含则返回false。
     * 
     * @param content
     * @return
     */
    public boolean checkCensorText(String content) {
        if (StringUtils.isBlank(content)) {
            return true;
        }

        try {
            return checkCensorWords(content);
        } catch (Exception e) {
            logger.error("Check censor text error.", e);
            return false;
        }
    }
